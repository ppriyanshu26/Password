...
class CredentialManagerGUI:
    ...
    def _create_main_screen(self):
        ...
        tk.Button(sel_frame, text="➕", command=self._add_platform, font=("Segoe UI", 10), bg=COLOR_ACCENT, fg=COLOR_BG_DARK, relief="flat", padx=8, pady=3).pack(side="left", padx=(0, 20))
        ...
        tk.Button(btn_frame, text="➕ Add", command=self._add_cred, font=("Segoe UI", 10), bg=COLOR_ACCENT, fg=COLOR_BG_DARK, relief="flat", padx=15, pady=8).pack(side="left", padx=5)
        tk.Button(btn_frame, text="✏️ Edit", command=self._edit_cred, font=("Segoe UI", 10), bg=COLOR_ACCENT, fg=COLOR_BG_DARK, relief="flat", padx=15, pady=8).pack(side="left", padx=5)
        tk.Button(btn_frame, text="🗑️ Delete", command=self._del_cred, font=("Segoe UI", 10), bg=COLOR_ERROR, fg="white", relief="flat", padx=15, pady=8).pack(side="left", padx=5)
        tk.Button(btn_frame, text="🔐 Change Master Key", command=self._change_master_key, font=("Segoe UI", 10), bg="#FFA500", fg="white", relief="flat", padx=15, pady=8).pack(side="left", padx=5)
    ...
    def _show_login_error(self):
        frame = self.root.winfo_children()[0] if self.root.winfo_children() else None
        if frame:
            for widget in frame.winfo_children(): widget.destroy()
            tk.Label(frame, text="❌ Invalid Password", font=("Segoe UI", 14, "bold"), fg=COLOR_ERROR, bg=COLOR_BG_DARK).pack(pady=20)
            tk.Button(frame, text="← Back", command=self._create_login_screen, font=("Segoe UI", 10), bg=COLOR_ACCENT, fg=COLOR_BG_DARK, padx=20, pady=10, relief="flat").pack(pady=20)
    
    def _show_message(self, title, message, on_back=None):
    ...
    def _show_confirmation(self, title, message, on_yes, on_no):
    ...
    def _show_input_dialog(self, title, fields, on_submit, on_cancel):
        ...
        self._render_input_dialog(title)
    
    def _render_input_dialog(self, title):
        ...
        self.detail_text.bind("<Escape>", cancel)
        self.detail_text.bind("<Key>", self._on_input_key)
        self.detail_text.focus()
    ...
    def _add_platform(self):
        def on_submit(values):
            ...
            if not platform: self._show_message("Error", "Platform name required"); return
            if platform in self.manager.get_platforms(): self._show_message("Error", "Platform already exists"); return
            self.manager.vault.vault[platform] = []
            self.manager.vault.save()
            self._show_message("Success", f"Added {platform}", lambda: self._create_main_screen())
        def on_cancel():
            self._clear_details()
        self._show_input_dialog("Add Platform", ["Platform Name"], on_submit, on_cancel)
    
    def _add_cred(self):
        if not self.current_platform: self._show_message("Error", "Select platform"); return
        def on_submit(values):
            u, pw, m = values.get("Username", "").strip(), values.get("Password", "").strip(), values.get("MFA (optional)", "").strip()
            if not all([u, pw]): self._show_message("Error", "Username and Password required"); return
            success, msg = self.manager.add_cred(self.current_platform, u, pw, m or None)
            if success: self._show_message("Success", msg, lambda: self._create_main_screen())
            else: self._show_message("Error", msg)
        def on_cancel():
            self._clear_details()
        self._show_input_dialog("Add Credential", ["Username", "Password", "MFA (optional)"], on_submit, on_cancel)
    def _edit_cred(self):
        if not self.current_credential: self._show_message("Error", "Select credential"); return
        def on_submit(values):
            pw, m = values.get("Password", "").strip(), values.get("MFA (optional)", "").strip()
            if not pw: self._show_message("Error", "Password required"); return
            success, msg = self.manager.edit_cred(self.current_platform, self.current_credential['username'], pw, m or None)
            if success: self._show_message("Success", msg, lambda: self._create_main_screen())
            else: self._show_message("Error", msg)
        def on_cancel():
            self._show_details()
        self._show_input_dialog("Edit Credential", ["Password", "MFA (optional)"], on_submit, on_cancel)
    ...
    def _del_cred(self):
        ...
        def on_no():
            self._show_details()
        self._show_confirmation("Confirm", f"Delete {self.current_credential['username']}?", on_yes, on_no)
    ...
    def _logout(self):
        ...
        def on_no():
            self._clear_details()
        self._show_confirmation("Logout", "Are you sure?", on_yes, on_no)
    ...
    def _change_master_key(self):
        def on_submit(values):
            ...
            if not current_pwd: self._show_message("Error", "Current master key required"); return
            if not new_pwd: self._show_message("Error", "New master key required"); return
            if new_pwd != confirm_pwd: self._show_message("Error", "Passwords don't match"); return
            
            if not cred.verify_master_key(current_pwd): self._show_message("Error", "❌ Incorrect current master key"); return
            
            cred.save_master_key(new_pwd)
            self.manager.crypto = Crypto(new_pwd)
            self.manager.vault.set_crypto(self.manager.crypto)
            self._show_message("Success", "Master key changed successfully", lambda: self._create_main_screen())
        
        def on_cancel():
            self._clear_details()
        
        self._show_input_dialog("Change Master Key", ["Current Master Key", "New Master Key", "Confirm New Master Key"], on_submit, on_cancel)
